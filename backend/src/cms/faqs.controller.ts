...
export class FaqsController {
  ...
  @Get()
  @ApiOperation({
    summary: 'Retrieve all general and medication FAQs',
    description: 'Lists all available collapsible questions and answers.',
  })
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Create new FAQ record (Admin only)',
    description: 'Saves collapsible item details.',
  })
  @ApiResponse({ status: 201, description: 'FAQ created successfully.' })
  async create(@Body() dto: CreateFaqDto, @RequestUser('sub') userId: string) {
    const data = await this.cmsService.createFaq(
      dto.question,
      dto.answer,
      dto.category,
      userId,
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Soft delete FAQ record (Admin only)',
    description: 'Disables FAQ item.',
  })
