...
export class AppointmentsController {
  ...
  @ApiOperation({
    ...
    description: 'Returns a list of appointments in the system.',
  })
  @ApiResponse({
    status: 200,
    description: 'Appointments queue fetched successfully.',
  })
  ...
  @Get(':id')
  @ApiOperation({
    summary: 'Get details of specific appointment',
    description: 'Retrieves appointment details by record ID.',
  })
  ...
  @ApiOperation({
    ...
    description: 'Sets the meeting link for a video consultation.',
  })
  async updateMeetingLink(
    @Param('id') id: string,
    @Body('meetingLink') meetingLink: string,
  ) {
    const data = await this.appointmentsService.updateMeetingLink(
      id,
      meetingLink,
    );
    return {
  ...
  @ApiOperation({
    ...
    description: 'Retrieves chat messages for a text consultation.',
  })
  ...
  @ApiOperation({
    ...
    description: 'Sends a text message in a chat consultation.',
  })
  async sendMessage(
    @Param('id') id: string,
  ...
  @ApiOperation({
    summary: 'Book a new consultation slot',
    description: 'Registers a new pending appointment slot.',
  })
  ...
  async create(
    ...
    const data = await this.appointmentsService.createForUser(
      userId,
      doctorId,
      appointmentDate,
      appointmentTime,
  ...
  @ApiOperation({
    ...
    description: 'Sets status to Confirmed, Completed, Cancelled, etc.',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    summary: 'Cancel and soft-delete appointment',
    description: 'Marks appointment as deleted.',
  })
  @ApiResponse({
    status: 200,
    description: 'Appointment cancelled and soft deleted successfully.',
  })
  ...
  @Roles(UserRole.Doctor)
  @ApiOperation({
    summary: 'Submit consultation report and complete session',
    description:
      'Restricted to assigned doctor. Creates clinical report, issues prescription, and updates status.',
  })
  @ApiResponse({
    status: 200,
