...
import { CreditCard, FileText, Download, CheckCircle, ArrowLeft, Building2, Stethoscope, TrendingUp, IndianRupee, Users } from "lucide-react";
    ...
    async function loadPayments() {
      try {
        ...
        if (res.success && Array.isArray(res.data)) {
          ...
          const records: PaymentRecord[] = paidAppointments.map((a: any) => ({
            id: a.id,
  ...
  const columns: TableColumn<PaymentRecord>[] = [
    {
      ...
      label: "Patient Name",
      sortable: true,
      render: (row) => <span className="font-bold text-text-primary text-sm">{row.patientName}</span>
    },
    {
      key: "amount",
      label: "Amount Paid",
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto animate-fade-in print-hide-container">
      ...
      <style dangerouslySetInnerHTML={{__html: `
        @media print {
          /* Hide all direct children of body (the main app) */
          body > * {
            display: none !important;
          }
          /* Show ONLY the modal wrapper, and format it for printing */
          body > .print-modal-wrapper {
          ...
          /* Reset the inner div that has rounded corners and shadow */
          ...
          /* Remove background colors that browsers strip, force them to print */
          * {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-text-secondary mt-0.5">
                Track payments, download patient receipts, and monitor your clinic revenue.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 print-hide">
        <div className="bg-gradient-to-br from-white to-slate-50 p-5 rounded-3xl border border-border/60 shadow-sm relative overflow-hidden group hover:shadow-md transition-all">
          ...
          <h3 className="text-3xl font-heading font-black text-text-primary">
            ₹{loading ? "..." : payments.reduce((sum, p) => sum + p.amount, 0).toLocaleString()}
          </h3>
        ...
        <div className="bg-gradient-to-br from-white to-slate-50 p-5 rounded-3xl border border-border/60 shadow-sm relative overflow-hidden group hover:shadow-md transition-all">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-xs font-bold text-text-secondary uppercase tracking-wider">Transactions</span>
          </div>
          <h3 className="text-3xl font-heading font-black text-text-primary">
            {loading ? "..." : payments.length}
          </h3>
          <p className="text-[10px] text-text-secondary font-medium mt-1">Successful payments logged</p>
        </div>
        ...
        <div className="bg-gradient-to-br from-white to-slate-50 p-5 rounded-3xl border border-border/60 shadow-sm relative overflow-hidden group hover:shadow-md transition-all">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-xs font-bold text-text-secondary uppercase tracking-wider">Unique Patients</span>
          </div>
          <h3 className="text-3xl font-heading font-black text-text-primary">
            {loading ? "..." : new Set(payments.map(p => p.patientName)).size}
          </h3>
          <p className="text-[10px] text-text-secondary font-medium mt-1">Patients billed this period</p>
        </div>
      </div>
      ...
      <div className="bg-white p-3 sm:p-5 border border-border/80 rounded-3xl shadow-sm max-w-full overflow-x-auto print-hide">
        {loading ? (
          ...
          <AdvancedTable
            ...
            columns={columns}
            rowKey={(r) => r.id}
            searchKeys={["patientName", "transactionId"]}
            searchPlaceholder="Search by patient name or transaction ID..."
          />
        )}
      </div>
      ...
      {selectedReceipt && typeof window !== 'undefined' && createPortal(
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm print-modal-wrapper">
          <div className="w-full max-w-lg bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col animate-slide-up relative">
            ...
            <div className="bg-slate-50 border-b border-border p-4 flex items-center justify-between print-hide">
              <h3 className="font-bold text-sm text-text-primary">PDF Receipt Preview</h3>
            ...
            <div id="receipt-modal-content" className="p-10 space-y-8 bg-white text-left relative overflow-hidden">
              ...
              <div className="flex justify-between items-start border-b-2 border-slate-100 pb-6 relative z-10">
                ...
                <div className="text-right space-y-1.5 pt-1">
                  ...
                  <p className="text-[11px] text-slate-500 font-medium">{user?.doctor?.specialization || "General Consultation"}</p>
                  <p className="text-[10px] text-slate-400">License: {user?.doctor?.licenseNumber || "N/A"}</p>
                </div>
              </div>
